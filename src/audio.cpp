...
{
    gou_audio_t* result = (gou_audio_t*)malloc(sizeof(*result));
    if (!result)
    {
    ...
    // testing
    //uint32_t vol = gou_audio_volume_get(result);
    //printf("audio: vol=%d\n", vol);
    //gou_audio_path_get(result);
