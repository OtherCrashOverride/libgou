...
static const char* EVDEV_NAME_2 = "/dev/input/by-path/platform-gou_joypad-event-joystick";
...
#define GOU_THUMBSTICK_COUNT (2)
#define GOU_BUTTON_COUNT (Go2InputButton_TriggerRight + 1)


typedef struct gou_input_state
{
    gou_thumb_t thumbs[GOU_THUMBSTICK_COUNT];
    gou_button_state_t buttons[GOU_BUTTON_COUNT];
} gou_input_state_t;

typedef struct gou_input
{
    int fd;
    struct libevdev* dev;
    gou_input_state_t current_state;
    gou_input_state_t pending_state;    
    pthread_mutex_t gamepadMutex;
    pthread_t thread_id;
    gou_battery_state_t current_battery;
...
{
    gou_input_t* input = (gou_input_t*)arg;
    int fd;
    void* result = 0;
    char buffer[BATTERY_BUFFER_SIZE + 1];
    gou_battery_state_t battery;
...
static void* input_task(void* arg)
{
	...
	while (!input->terminating)
	{
		...
		if (rc == 0)
		{
            ...
            if (ev.type == EV_KEY)
			{
                gou_button_state_t state = ev.value ? ButtonState_Pressed : ButtonState_Released;
...
gou_input_t* gou_input_create()
{
    ...
    gou_input_t* result = (gou_input_t*)malloc(sizeof(*result));
    if (!result)
    {
...
{
    gou_input_state_t* result = (gou_input_state_t*)malloc(sizeof(*result));
    if (!result)
    {
        printf("malloc failed.\n");
        abort();
